...
# echo -e '\E[COLOR1;COLOR2m\033[StylemMensagem.'
# \033[1m bold
# \033[4m underline
# \033[0m offAtributte
#
...
#
#\E[31;40m\033[1m (vermelho)
#\E[32;40m\033[1m (verde)
...
declare oldIFS=$IFS
declare IFS=$'\n'
# funcao caminhoCompleto
declare dirDiscos="/media/DISC*"
declare dirNiveis=5

# funcao aguardarTempo
declare aguardarTempo=0
declare aguardarTempoMax=9
...
function caminhosCompletos () {
	caminhosCompletos=""
	caminhosCompletos=(`find -L $dirDiscos -mindepth 1 -maxdepth 2 -type d | grep OsMeusFilmes`)
	echo Numero de Pastas para processar=${#caminhosCompletos[@]}
	}

function aguardarTempo () {
	# Coloca o script em espera entre 0 e 60 seg.
	aguardarTempoMax=$1
...
function IMDBidentificarCode () {

	codeIMDB=$(cat $caminhoCompleto/filme.txt | grep "<id moviedb=imdb>" | sed -e :a -e 's/<[^>]*>//g;s/ //g;s/\t//g')
	xbmcDestinoHtml="$caminhoCompleto/html"

# INI Procurar o filme no Google IMDB
	if [[ $codeIMDB == "" ]]; then
		echo "Procurar o filme no Google IMDB ..."
		if [[ $renovarHTML == 1 ]]; then
			echo -e '\E[32;40m\033[1mDescarregando HTML de http://www.google.com/search?hl=en&q='$FILME'+imdb ...\033[0m'
			aguardarTempo 5
		...
		if [ $? -ne 0 ]; then
			echo -e '\E[31;40m\033[1mA ligação a Google falhou. Verifique a sua ligação a internet!\033[0m'
			exit 192
		fi
	fi
# END Procurar o filme no Google IMDB

# INI Identificar o endereços IMDB para o filme
	echo -e '\E[32;40m\033[1mIdentificar o endereços IMDB para o filme...\033[0m'
	if [[ $codeIMDB == "" ]]; then
		URLIMDB=`egrep -o "http://www.imdb.com/title/tt[0-9]*/" $xbmcDestinoHtml/google_imdb.html | head -1`
		codeIMDB=$(echo $URLIMDB | sed -e 's/http//g;s/tt/\ntt/g;s/\///g' | grep tt)
	else
		URLIMDB=$(echo -e "http://www.imdb.com/title/$codeIMDB/")
	fi
	echo -e '\E[32;40m\033[1m'$URLIMDB'\033[0m'
	URLIMDBC="${URLIMDB}combined"
# END Identificar o endereços IMDB para o filme

	if [[ $codeIMDB == "" ]]; then
		echo -e '\E[31;40m\033[1mO processamento do filme '$FILME'\033[0m'
		echo -e '\E[31;40m\033[1mfoi cancelado porque o codigo IMDB é desconhecido\033[0m'
		echo -e '\E[31;40m\033[1m'$caminhoCompleto'\033[0m'
		exit 0
	else
		echo -e '\E[32;40m\033[1mVai ser processado o filme '$FILME'\033[0m'
		echo -e '\E[32;40m\033[1mcom o cod IMDB:'$codeIMDB'\033[0m'
	fi
	}

function IMDBguardarData () {
	
	IMDBidentificarCode
	
	# INI Guardar os detalhes do filme IMDB
	echo -e '\E[32;40m\033[1mGuardar os detalhes do filme IMDB ...\033[0m'
	if [[ $renovarHTML == 1 ]]; then
		echo -e '\E[32;40m\033[1mDescarregando HTML de '$URLIMDB' ...\033[0m'
		aguardarTempo 5
		lynx --source ${URLIMDB} > $xbmcDestinoHtml/imdb.html;
		echo -e '\E[32;40m\033[1mDescarregando HTML de '$URLIMDBC' ...\033[0m'
		aguardarTempo 5
		lynx --source ${URLIMDBC} | sed -e 's/<\/div>/<\/div>\n/g' -e 's/<\/li>/<\/li>\n/g' -e 's/<\/tr>/<\/tr>\n/g' -e 's/<\/table>/<\/table>\n/g' -e 's/\&nbsp\;/ /g' -e 's/\&raquo\;//g' -e 's/<br>/<br>\n/g'> $xbmcDestinoHtml/imdb_combined.html;
		echo -e '\E[32;40m\033[1mDescarregando JSON para'$codeIMDB' ...\033[0m'
		aguardarTempo 5
		curl -s --request GET 'http://mymovieapi.com/?id='$codeIMDB'&type=json&plot=simple&episode=1&lang=en-US&aka=full&release=full&business=0&tech=1' > $xbmcDestinoHtml/imdb.json
	else
...
# END Guardar os detalhes do filme IMDB
	
	}

function IMDBprocessarData () {
	
	IMDBguardarData

# INI Extrair informação (parse) CODIGO IMDB
echo -e '\E[32;40m\033[1mExtrair informação (parse) CODIGO IMDB...\033[0m'
ImdbCode=$codeIMDB
# END Extrair informação (parse) CODIGO IMDB

# INI Extrair informação (parse) ANO IMDB
ANO=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "0,\"year" | grep -o "[0-9][0-9][0-9][0-9]"`
if [[ "$ANO" == "" ]]; then
...
# END Extrair informação (parse) ANO IMDB

# INI Extrair informação (parse) TITULO IMDB
TITULO=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "\[\"title\"\]" | awk -F'\t' '{ print $2 } | sed 's/.$//;s/^.//'`
limparUnicode "$TITULO"
TITULO=`echo -e "$limparUnicode"`
if [[ "$TITULO" == "" ]]; then
...
# END Extrair informação (parse) TITULO IMDB

# INI Extrair informação (parse) RAT IMDB
RAT=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "\[\"rating\"\]" | awk -F'\t' '{ print $2 }'`
if [[ "$RAT" == "" ]]; then
...
# END Extrair informação (parse) RAT IMDB

# INI Extrair informação (parse) VOTES IMDB
VOTES=`cat $xbmcDestinoHtml/imdb_combined.html | sed -n '/<a href="ratings" class="tn15more">/,/<\/div>/{s/<[^>]*>//g;p;}'`
VOTES=$(echo $VOTES | sed 's/\&nbsp;//g;s/\&raquo;//g;s/ //g;/^$/d;s/votes//g;s/\,//g;s/^\s*//;s/\s*$//g')
...
# END Extrair informação (parse) VOTES IMDB

# INI Extrair informação (parse) MPAA IMDB
...
# END Extrair informação (parse) MPAA IMDB

# INI Extrair informação (parse) RUNTIME IMDB
RUNTIME=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep ",\"runtime\"" | awk -F'\t' '{ print $2 }' | sed 's/.$//;s/^.//'`
if [[ $RUNTIME == "" ]]; then
...
# END Extrair informação (parse) RUNTIME IMDB

# INI Extrair informação (parse) STUDIO IMDB
...
# END Extrair informação (parse) STUDIO IMDB

# INI Extrair informação (parse) COUNTRY IMDB
COUNTRY=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "\[\"country\"" | awk -F'\t' '{ print $2 }' | sed 's/.$//;s/^.//'`
if [[ $COUNTRY == "" ]]; then
...
# END Extrair informação (parse) COUNTRY IMDB

# INI Extrair informação (parse) PREMIERED IMDB
...
# END Extrair informação (parse) PREMIERED IMDB

# INI Extrair informação (parse) DIR IMDB
DIR=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "\"directors\"" | awk -F'\t' '{ print $2 }' | sed 's/.$//;s/^.//'`
if [[ $DIR != "" ]]; then
...
# END Extrair informação (parse) DIR IMDB

# INI Extrair informação (parse) WRITER IMDB
WRITER=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "\"writers\"" | awk -F'\t' '{ print $2 }' | sed 's/.$//;s/^.//'`
if [[ $WRITER != "" ]]; then
...
# END Extrair informação (parse) WRITER IMDB

# INI Extrair informação (parse) GEN IMDB
GEN=`cat $xbmcDestinoHtml/imdb.json | json.sh -b | grep "\"genres\"" | awk -F'\t' '{ print $2 }' | sed 's/.$//;s/^.//'`
if [[ $GEN != "" ]]; then
	GEN=`cat $xbmcDestinoHtml/imdb_combined.html | sed -n '/Genre:/,/<\/div>/{s/Genre://;p}' | sed -e :a -e 's/<[^>]*>//g;/</N;//ba'| sed 's/ See more //' | sed '/^$/d'`
	limparCodeHTML "$GEN"
	GEN="$limparCodeHTML"
fi
# END Extrair informação (parse) GEN IMDB

# INI Extrair informação (parse) CAST IMDB
CAST=$(cat $xbmcDestinoHtml/imdb_credit.html | sed -n '/> Cast</,/rest of cast/{p}' | sed -e 's/<td class="nm">/\nActor:/g' | sed -e 's/<td class="char">/\;Papel:/g' | sed -e :a -e 's/<[^>]*>//g;/</N;//ba' | grep "Actor:" | sed 's/ ... //g' | sed 's/\.\.\.//g')
limparCodeHTML "$CAST"
echo $limparCodeHTML | sed 's/Actor/\nActor/g' > $CASTFILEIMDB
CAST=$(cat $CASTFILEIMDB | sed -e 's/`/ /g;')
CAST=( `echo "$CAST" | sed -e 's/  / /g;s/^\s*//;s/\s*$//g'` )
# END Extrair informação (parse) CAST IMDB
...
function limparUnicode () {
	limparUnicode=$(echo "$1" | sed -e '{
						s/\\u00e1/\á/g
						s/\\u00e0/\à/g
						s/\\u00e2/\â/g
						s/\\u00e3/\ã/g
