...
function wortenMirror () {
    ...
    #onOff3G on
        ...
        #antigo
        # httrack -q -%i -iC2 http://www.worten.pt/store/inicio/eletrodomesticos.html -O "$dirProcesWorten/" -n -%P -p7 -N0 -s2 -%U francisco -x -p7 -D -a -K0 -c10 -%k -A25000 -F "$useragent" -%F '<!-- Mirrored from %s%s by HTTrack Website Copier/3.x [XR&CO 2008] -->' -*.png -*.gif -*.jpg +*.css +*.js -ad.doubleclick.net/* -*AddBasket.aspx* -www.tlantic.com.br/* -*.png.image* -*.bmp.image* -*.tif.image* -*.gif.image* -*.jpg.image* -%s -%u
        #novo
        # httrack -q -%i -iC1 http://www.worten.pt/store/inicio/eletrodomesticos.html http://www.worten.pt/store/inicio/imagem-e-som.html http://www.worten.pt/store/inicio/informatica.html http://www.worten.pt/store/inicio/fotografia.html http://www.worten.pt/store/inicio/musica-e-filmes.html http://www.worten.pt/store/inicio/worten-mobile.html http://www.worten.pt/store/inicio/worten-gamer.html -O "$dirProcesWorten/" -n -%P -N0 -s2 -p7 -D -a -K0 -c4 -%k -A25000 -F "$useragent" -%F "<!-- Mirrored from %s%s by HTTrack Website Copier/3.x [XR&CO'2008], %s -->"  -*.png -*.gif -*.jpg +*.css +*.js -media.worten.pt -*AddBasket.aspx* -*.png.image* -*.bmp.image* -*.tif.image* -*.gif.image* -*.jpg.image* -%s -%u
        httrack -q -%i -iC2 http://www.worten.pt/store/inicio/eletrodomesticos.html -O "$dirProcesWorten/" -n -%P -p7 -N0 -s2 -%U francisco -r9999 --ext-depth1 -x -p7 -D -a -K0 -c4 -%k -A25000 -F "$useragent" -%F "<!-- Mirrored from %s%s by HTTrack Website Copier/3.x [XR&CO'2008], %s -->"  -*.png -*.gif -*.jpg -*.jpeg +*.css +*.js -*media.worten.pt -googleads.g.doubleclick.net -*AddBasket.aspx* -*.png.image* -*.bmp.image* -*.tif.image* -*.gif.image* -*.jpg.image* -%s -%u
        #onOff3G off
        #echo $ESTADO $(date +%Y-%m-%d_%H:%M) >> "$dirProcesWorten"/precos.log
...
function wortenProductDetail () {
    if [[ "$mirror" == "ok" ]]; then
        ...
        #IFS=$'\n'
        cd $dirDestWorten
        #>  /home/francisco/websites/worten/ficheiros.txt
        #find /home/francisco/websites/worten | grep html | grep "www.worten.pt" >> /home/francisco/websites/worten/ficheiros.txt
        ARRAY=(`find /home/francisco/websites/worten | grep html | grep "www.worten.pt"`)
        #ARRAY=($(cat /home/francisco/websites/worten/ficheiros.txt))
        COUNT=${#ARRAY[@]}
        volta=1
        csv=""
        df=""
        ...
        if [[ -f "$dirProcesWorten/$csv" ]]; then
            ...
            echo "ID;EAN;Cod.;Familia;Nome;Marca;PVP;Stock;Link" > "$dirProcesWorten/$csv"
            for i in ${ARRAY[*]}; do
                file="$i"
                echo $file
                echo -e $file >> "$dirProcesWorten/dataLayer.txt"
#                echo "Transforme em uma linha o ficheiro"
                 # bommmm dataLayer=$(cat $file | sed -n '/dataLayer =/,/\/script/{p}' | sed -e 's/<[^>]*>//g;s/'\'','\''/\\n/g;s/'\'', '\''/\\n/g;s/{/\\n/g;s/}/\\n/g;s/'\''//g')
                 # dataLayer=$(cat $file | sed -n '/dataLayer =/,/\/script/{p}' | sed -e 's/<[^>]*>//g' | sed -e "s/,\'/#/g" | sed -e 's/#/\\n/g' | sed -e 's/{/\\n/g')
                 dataLayer=$(cat $file | sed -n '/dataLayer =/,/\/script/{p}' | sed -e 's/<[^>]*>//g' | sed -e "s/,\'/#/g" | sed -e 's/#/\\n/g' | sed -e 's/'\'','\''/\\n/g;s/'\'', '\''/\\n/g;s/{/\\n/g;s/}/\\n/g;s/'\''//g')
#                umalinha=$(cat $file | tr -d "\015" | tr -d '\n')
#                echo "Muda de linha dataLayer"
#                dataLayer3=$(echo $umalinha | sed -e 's/dataLayer\ \=/\ndataLayer\ \=/g')
#                echo $dataLayer3 | grep -B 1  "dataLayer"
#                echo "Muda de linha Script"
#                dataLayer2=$(echo $dataLayer3 | sed -e 's/<\/script>/\n<\/script>/g')
#                echo $dataLayer3 | grep -B 1  "dataLayer"
#                echo "Encontra dataLayer com grep"
#                dataLayer1=$(echo $dataLayer2 | grep "dataLayer =")
                echo -e $dataLayer
                echo -e $dataLayer >> "$dirProcesWorten/dataLayer.txt"
            done
...
function exeSempre () {
    . /home/francisco/ownCloud/Linux/OsMeusScripts/precos.cfg
    while [ $precos = 1 ]
    do
        tempo
        useragent
