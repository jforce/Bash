...
{
	if [ -f cinelerra-ppa-cinelerra-exp-$CODENAME.list ]
	then
...
function Flash()
{
	...
	then
		...
		if [ $ARCHITECTURE = "x86_64" ]
		then
			wget -nc http://download.macromedia.com/pub/labs/flashplayer10/flashplayer10_2_p3_64bit_linux_111710.tar.gz
...
function VirtualBox()
{
	...
	if [ $ARCHITECTURE = "i686" ]
	then
		wget -nc http://download.virtualbox.org/virtualbox/4.0.8/virtualbox-4.0_4.0.8-71778~Ubuntu~$CODENAME\_i386.deb
	fi
	if [ $ARCHITECTURE = "x86_64" ]
	then
		wget -nc http://download.virtualbox.org/virtualbox/4.0.8/virtualbox-4.0_4.0.8-71778~Ubuntu~$CODENAME\_amd64.deb
	fi
	...
	wget -nc http://dlc.sun.com.edgesuite.net/virtualbox/4.0.6/Oracle_VM_VirtualBox_Extension_Pack-4.0.6-71344.vbox-extpack
	echo "=================================================="
...
then
	echo -e  $RED"Sorry, you are using Ubuntu $CODENAME.  Only Ubuntu $DISTROBUTION is supported.  Please check the files listed at http://sourceforge.net/projects/bleedingedge/files/ for your distrobution.  Program Terminated"$ENDCOLOR
	/usr/bin/notify-send "Sorry, you are using Ubuntu $CODENAME.  Only Ubuntu $DISTROBUTION is supported.  Please check the files listed at http://sourceforge.net/projects/bleedingedge/files/ for your distrobution.  Program Terminated"
	sleep 10
	exit 1
fi
if [ $ARCHITECTURE != "i686" ]
