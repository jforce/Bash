...
ANO=`$CAT $destinoHTML/imdb_combined.html | $GREP "<title>" | $SED -e :a -e 's/<[^>]*>//g;/</N;//ba' | $SED -e 's/\///g' | $AWK -F'(' '{print $2}' | $EGREP -o "[0-9][0-9][0-9][0-9]"`
limparCodeHTML "$ANO"
ANO="$limparCodeHTML"
#									  #
# Extrair informação (parse) ANO IMDB #
...
limparCodeMSDOS "$TITULO"
TITULOMSDOS="$limparCodeMSDOS"
#										 #
# Extrair informação (parse) TITULO IMDB #
##########################################
...
function limparCodeMSDOS () {
	limparCodeMSDOS=$(echo -e "$1" | $SED -e 's/\://g' | $SED -e 's/\\//g' | $SED -e 's/\///g' | $SED -e 's/\*//g' | $SED -e 's/\?//g' | $SED -e 's/\"//g' | $SED -e 's/\<//g' | $SED -e 's/\>//g' | $SED -e 's/\|//g')
	}
...
if [ $faltaFicheiro = 0 ] ; then
	...
	# echo -e "A processar o conteudo da pasta:\n$caminhoCompleto"

	# IDENTIFICAR CODIGO IMDB
	codeIMDB=$($CAT $caminhoCompleto/mymovies.xml | grep "<IMDbId>" | $SED -e 's/<IMDbId>//g' | $SED -e 's/<\/IMDbId>//g' | $SED -e 's/.$//g' | $SED -e 's/\t//g' | $SED -e 's/ //g')
	# IDENTIFICAR CODIGO IMDB
	...
	xbmcDestinoNfo=$(echo -e nfo/"$TITULOMSDOS".nfo)
...
for CONTADOR in $(seq 0 $((${#CAST[*]} - 1))); do
	...
	if [[ ! -f $HOME/People/$xbmcActorChar/$xbmcACTOR/folder.jpg ]]; then
		...
		xbmcURLACTORJPG=`$CAT $destinoHTML/google_$xbmcACTOR.html | $SED 's/imgurl/\nimgurl/g' | $SED 's/\&imgrefurl/\n&imgrefurl/g' | $EGREP imgurl | $SED 's/imgurl\=//g' | $EGREP jpg | $HEAD -1`
		wget -O - "$xbmcURLACTORJPG" > $HOME/People/$xbmcActorChar/$xbmcACTOR/folder.jpg 2> /dev/null;
		#rm "$destinoHTML/imdb_$xbmcActor.html" > /dev/null
	fi
done
