...
function menu () {
	...
	case $opcao in g|G)
	echo -e '\E[32;40m\033[1mA executar opção:[G] Executar opções N F B T\033[0m'
	BARRELA=1
	dirName
	nfoArrumar
	gerarNfoData
...
if [[ $codeTMDB == "" ]]; then
	if [[ $renovarHTML == 1 ]]; then
		...
		lynx -connect_timeout=10 --source "http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/$APITMDB/$codeIMDB" > $xbmcDestinoHtml/tmdb.xml 2> /dev/null
	else
...
if [[ $codeTMDB == "" ]]; then
	encontraTMDB=$(cat $xbmcDestinoHtml/tmdb.xml | grep "<movies>" | sed -e :a -e 's/<[^>]*>//g;s/ //g')
	if [[ $encontraTMDB != "Nothing found." ]]; then
	codeTMDB=$(cat $xbmcDestinoHtml/tmdb.xml | grep "<id>" | sed -e :a -e 's/<[^>]*>//g;s/ //g')
	fi
fi
...
encontraTMDB=$(cat $xbmcDestinoHtml/tmdb.xml | grep "<movies>" | sed -e :a -e 's/<[^>]*>//g;s/ //g')
if [[ $encontraTMDB != "Nothing found." ]]; then
	ptTMDB=$(cat $xbmcDestinoHtml/tmdb.xml | grep "<translated>" | sed -e :a -e 's/<[^>]*>//g;s/ //g')
	if [[ $ptTMDB == "true" ]]; then
		PLOTTMDB=`cat $xbmcDestinoHtml/tmdb.xml | grep "<overview>" | sed -e :a -e 's/<[^>]*>//g;s/^......//g'`
...
imgUrlPOSTER=$(cat $xbmcDestinoHtml/tmdb.xml | grep 'type="poster"' | grep jpg | grep "\original" | sed 's/.*url="//g;s/" size=.*//')
...
imgUrlBACKDROP=$(cat $xbmcDestinoHtml/tmdb.xml | grep 'type="backdrop"' | grep jpg | grep "\original" | sed 's/.*url="//g;s/" size=.*//')
