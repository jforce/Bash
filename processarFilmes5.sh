...
if [[ $codeTMDB == "" ]]; then
	if [[ $renovarHTML == 1 ]]; then
		echo "Descarregando XML de http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/$APITMDB/$codeIMDB ..."
		$LYNX -connect_timeout=10 --source "http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/$APITMDB/$codeIMDB" > $destinoHTML/tmdb.xml 2> /dev/null
	else
		if [[ ! -f "$destinoHTML/TMDB.xml" ]]; then
			echo "Descarregando XML de http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/$APITMDB/$codeIMDB ..."
			$LYNX -connect_timeout=10 --source "http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/$APITMDB/$codeIMDB" > $destinoHTML/tmdb.xml 2> /dev/null
		fi
	fi
	...
	if [[ $renovarHTML == 1 ]]; then
		echo "Descarregando XML de http://api.themoviedb.org/2.1/Movie.getInfo/pt/xml/$APITMDB/$codeTMDB ..."
		$LYNX -connect_timeout=10 --source "http://api.themoviedb.org/2.1/Movie.getInfo/pt/xml/$APITMDB/$codeTMDB" > $destinoHTML/tmdb.xml 2> /dev/null
	else
		if [[ ! -f "$destinoHTML/TMDB.xml" ]]; then
			echo "Descarregando XML de http://api.themoviedb.org/2.1/Movie.getInfo/pt/xml/$APITMDB/$codeTMDB ..."
			$LYNX -connect_timeout=10 --source "http://api.themoviedb.org/2.1/Movie.getInfo/pt/xml/$APITMDB/$codeTMDB" > $destinoHTML/tmdb.xml 2> /dev/null
...
# Identificar codeTMDB caso nao exista
if [[ $codeTMDB == "" ]]; then
	encontraTMDB=$($CAT $destinoHTML/tmdb.xml | $GREP "<movies>" | $SED -e :a -e 's/<[^>]*>//g;s/ //g')
	if [[ $encontraTMDB != "Nothing found." ]]; then
	codeTMDB=$($CAT $destinoHTML/tmdb.xml | $GREP "<id>" | $SED -e :a -e 's/<[^>]*>//g;s/ //g')
	fi
fi

# Extrair informação (parse) PLOT TMDB
encontraTMDB=$($CAT $destinoHTML/tmdb.xml | $GREP "<movies>" | $SED -e :a -e 's/<[^>]*>//g;s/ //g')
if [[ $encontraTMDB != "Nothing found." ]]; then
	ptTMDB=$($CAT $destinoHTML/tmdb.xml | $GREP "<translated>" | $SED -e :a -e 's/<[^>]*>//g;s/ //g')
	if [[ $ptTMDB == "true" ]]; then
		PLOTTMDB=`$CAT $destinoHTML/tmdb.xml | $GREP "<overview>" | $SED -e :a -e 's/<[^>]*>//g;s/^......//g'`
...
# Processar Posters do TMDB
imgUrlPOSTER=$($CAT $destinoHTML/tmdb.xml | $GREP 'type="poster"' | $GREP jpg | $GREP "\-original" | $SED 's/.*url="//g;s/" size=.*//')
if [[ $imgUrlPOSTER != "" ]]; then
imgUrlPOSTER=( `$CAT $destinoHTML/tmdb.xml | $GREP 'type="poster"' | $GREP jpg | $GREP "\-original" | $SED 's/.*url="//g;s/" size=.*//'` )
	for CONTADOR in $($SEQ 0 $((${#imgUrlPOSTER[*]} - 1))); do
		if [[ ! -f "$caminhoCompleto/poster$CONTADOR.jpg" ]]; then
		aguardarTempo 5
		echo "$caminhoCompleto/poster$CONTADOR.jpg"
		$WGET --timeout 30 -t 1 -O - "${imgUrlPOSTER[$CONTADOR]}" > $caminhoCompleto/poster$CONTADOR.jpg 2> /dev/null;
		fi
	done
fi
...
# Processar Backdrops do TMDB
if [[ -f $caminhoCompleto/backdrop.jpg ]]; then
mv "$caminhoCompleto/backdrop.jpg" "$caminhoCompleto/backdrop0.jpg"
fi

imgUrlBACKDROP=$($CAT $destinoHTML/tmdb.xml | $GREP 'type="backdrop"' | $GREP jpg | $GREP "\-original" | $SED 's/.*url="//g;s/" size=.*//')
if [[ $imgUrlBACKDROP != "" ]]; then
imgUrlBACKDROP=( `$CAT $destinoHTML/tmdb.xml | $GREP 'type="backdrop"' | $GREP jpg | $GREP "\-original" | $SED 's/.*url="//g;s/" size=.*//'` )
	for CONTADOR in $($SEQ 0 $((${#imgUrlBACKDROP[*]} - 1))); do
		if [[ ! -f "$caminhoCompleto/backdrop$CONTADOR.jpg" ]]; then
		aguardarTempo 5
		echo "$caminhoCompleto/backdrop$CONTADOR.jpg"
		$WGET --timeout 30 -t 1 -O - "${imgUrlBACKDROP[$CONTADOR]}" > $caminhoCompleto/backdrop$CONTADOR.jpg 2> /dev/null;
		fi
	done
fi
...
#http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/2139fe2ff8e32e6081db53edaf2b18af/tt0353515 false
#http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/2139fe2ff8e32e6081db53edaf2b18af/tt0137523 true
#http://api.themoviedb.org/2.1/Movie.imdbLookup/pt/xml/2139fe2ff8e32e6081db53edaf2b18af/tt1299999 invalido
...
testeFontePLOT=$(echo -e $PLOT | $GREP -o Fonte:IMDB)
...
function nfoArrumar () {
	...
	nfoProcessados=0
	for caminhoCompleto in $($FIND -L $dirDiscos -mindepth 1 -maxdepth $dirNiveis -type f -name "movie.nfo" | $GREP VIDEO_TS | $SED -e '/OsMeusFilmesInfantis/ d')
	do
	nfoProcessados=1
...
if [ $faltaFicheiro = 0 ] ; then
	...
	for caminhoCompleto in $($FIND -L $dirDiscos -mindepth 1 -maxdepth $dirNiveis -type f -name "mymovies.xml" | $GREP mymovies.xml | $SED -e 's/\/mymovies.xml//g' -e 's/  //g' )
	do	
	...
	# IDENTIFICAR CODIGO IMDB
	#
	# IDENTIFICAR CODIGO TMDB
		codeTMDB=$($CAT $caminhoCompleto/mymovies.xml | $GREP "<TMDbId>" | $SED -e :a -e 's/<[^>]*>//g;s/ //g;s/\t//g')
	# IDENTIFICAR CODIGO TMDB
	#
	# IDENTIFICAR DESCRICAO MANUAL
...
#############
# Versão com actores seprados por pastas por letras e com pasta para cada actor com folder.jpg
#
#	xbmcActorChar=$(echo -e $xbmcACTOR | $SED -e 's/^\(.\{1\}\).*$/\1/' | $TR '[:lower:]' '[:upper:]')#
#	if [[ ! -f $xbmcDestino/People/$xbmcActorChar/$xbmcACTOR/folder.jpg ]]; then
#		mkdir -p $xbmcDestino/People/$xbmcActorChar/$xbmcACTOR
#		if [[ $renovarHTML == 1 ]]; then
#		aguardarTempo 5
...
#		$WGET --timeout 30 -t 1 -O - "$xbmcURLACTORJPG" > $xbmcDestino/People/$xbmcActorChar/$xbmcACTOR/folder.jpg 2> /dev/null;
#		$FIND -L $xbmcDestino/People/$xbmcActorChar/$xbmcACTOR/ -type f -empty -exec rm -f {} \;
#	fi
#if [[ -f "$xbmcDestino/People/$xbmcActorChar/$xbmcACTOR/folder.jpg" ]]; then
#echo -e "\t<thumb>smb://$USERNAME:$PASSWORD@192.168.1.1$xbmcDestino/People/$xbmcActorChar/$xbmcACTOR/folder.jpg</thumb>" >> $xbmcDestinoNfo
#############
#
		mkdir -p $xbmcDestino/people/
		if [[ $renovarHTML == 1 ]]; then
		aguardarTempo 5
		echo "Descarregando HTML de http://images.google.com/images?q=actor $xbmcACTOR&imgtype=face ..."
		$LYNX -connect_timeout=10 --source "http://images.google.com/images?q=actor $xbmcACTOR&imgtype=face" > $destinoHTML/google_$xbmcACTOR.html 2> /dev/null;
		...
		xbmcURLACTORJPG=$($CAT $destinoHTML/google_$xbmcACTOR.html | $SED 's/imgurl/\nimgurl/g' | $SED 's/\&imgrefurl/\n&imgrefurl/g' | $EGREP imgurl | $SED 's/imgurl\=//g' | $EGREP jpg | $HEAD -1 | $SED 's/<img/\n<img/g' | $SED 's/.jpg/.jpg\n/g' | $SED -n 1p)
		if [[ ! -f "$xbmcDestino/people/$xbmcACTOR.jpg" ]]; then
		echo "Descarregando Imagem de $xbmcURLACTORJPG ..."
		$WGET --timeout 30 -t 1 -O - "$xbmcURLACTORJPG" > $xbmcDestino/people/$xbmcACTOR.jpg 2> /dev/null;
		$FIND -L $xbmcDestino/people/ -type f -empty -exec rm -f {} \;
		fi
if [[ -f "$xbmcDestino/people/$xbmcACTOR.jpg" ]]; then
xbmcDestinoSMedia=$(echo -e $xbmcDestino | $SED 's/\/media//g' )
echo -e "\t\t<thumb>smb://$USERNAME:$PASSWORD@192.168.1.2$xbmcDestinoSMedia/people/$xbmcACTOR.jpg</thumb>" >> $xbmcDestinoNfo
echo -e "\t</actor>" >> $xbmcDestinoNfo
#
#############
else
...
function backdrop () {
	...
	for caminhoCompleto in $($FIND -L "$dirDiscos" -maxdepth $dirNiveis -type d | $GREP OsMeusFilmes | $SED -e '/OsMeusFilmesInfantis/ d')
	do
			if [[ ! -f "$caminhoCompleto/backdrop.jpg" ]]; then
				for fileFILME in $(
					$FIND -L "$caminhoCompleto/" -mindepth 1 -maxdepth 1 -type f \
...
function tbn () {
	...
	if [ $faltaFicheiro = 0 ] ; then
		echo -e '\E[1;37m\033[1mProcessando as pastas ...\033[0m'
		#for caminhoCompleto in $($FIND -L $dirDiscos -mindepth 1 -maxdepth $dirNiveis -type f -name "mymovies.xml" | $GREP mymovies.xml | $SED -e 's/\/mymovies.xml//g' -e 's/  //g' | $GREP OsMeusFilmes | $SED -e '/OsMeusFilmesInfantis/ d')
		for caminhoCompleto in $($FIND -L $dirDiscos -mindepth 1 -maxdepth $dirNiveis -type f -name "movie.nfo" | $GREP movie.nfo | $SED -e 's/\/movie.nfo//g' -e 's/  //g' | $GREP OsMeusFilmes | $SED -e '/OsMeusFilmesInfantis/ d')
		do
		#filme=$($CAT $caminhoCompleto/mymovies.xml | $GREP LocalTitle | $SED -e 's/<LocalTitle>//g' -e 's/<\/LocalTitle>//g' -e 's/.$//g' -e 's/://g'  -e 's/?//g' -e 's/\&amp\;/\&/'  -e 's/  //g')
		filme=$($CAT $caminhoCompleto/movie.nfo | $GREP "<title>" | $SED -e 's/<title>//g' -e 's/<\/title>//g' -e 's/\t//g')
		#ano=$($CAT $caminhoCompleto/mymovies.xml | $GREP ProductionYear | $SED -e 's/<ProductionYear>//g' | $SED -e 's/<\/ProductionYear>//g' | $SED -e 's/.$//g' -e 's/^M//g' -e 's/  //g')
		...
		pastaSFilme=$(echo "$caminhoCompleto" | $SED 's/\(.*\)./\1/' | $AWK -F'/' '{for(i=1;i<NF-1;++i)printf $i"/";print $i}'| $SED -e 's/  //g')
			...
			if [[ ! -f "$fileFILMEsEXT.tnb" ]]; then
			tnbProcessados=1
			echo -e '\E[1;37m\033[1m##############\033[0m'
			echo -e '\E[1;37m\033[1m# Origem:'"$caminhoCompleto/movie.tbn"'\033[0m'
			echo -e '\E[1;37m\033[1m# Destino:'"$fileFILMEsEXT.tnb"'\033[0m'
			echo -e '\E[1;37m\033[1m##############\033[0m'
			echo ""			
			cp "$caminhoCompleto/movie.tbn" "$fileFILMEsEXT.tnb"
			fi
			done
		done
		else
